...
def LoadConfig_bestfinishes(aseco: "Aseco"):
    candidates = [
        Path(getattr(aseco, "_base_dir", ".")).resolve() / "bestfinishes.xml",
        Path(".").resolve() / "bestfinishes.xml",
    ]
    path = None
    ...
    if path is None:
        logger.warning("[BestFinishes] bestfinishes.xml not found, using defaults")
        return
    ...
    try:
        ...
        _state.config.nb_bestfinishes = int(
            _text("nb_bestfinishes", str(_state.config.nb_bestfinishes))
        )
        logger.info("[BestFinishes] Config loaded from %s", path)
    except Exception as exc:
        logger.warning("[BestFinishes] Failed to parse bestfinishes.xml: %r", exc)
