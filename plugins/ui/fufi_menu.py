...
- Loads menu ML template from plugins/ui/fufi/fufi_menu.xml
...
def _resolve_indicator_func(name: str):
    ...
    for mod_name, module in list(sys.modules.items()):
        if not module:
            continue
        if not mod_name.startswith("pyxaseco.plugins."):
            continue
...
def _resolve_chat_admin_auth_check():
    for mod_name, module in list(sys.modules.items()):
        if not module:
            continue
        if mod_name != "pyxaseco.plugins.chat.admin":
            continue
...
class FufiMenu:
    ...
    def _load_plugin_list(self):
        plugins_toml = Path("plugins.toml")
        if plugins_toml.exists():
            try:
                ...
                logger.warning("[FufiMenu] Could not parse plugins.toml: %s", e)
    ...
    def load_settings(self):
        ...
        self.caption = (str(self.config_root.get("menu_caption", "Menu")) or "Menu").strip() or "Menu"
        self.menutimeout = int(self.config_root.get("menutimeout", "0"))

        template_path = Path(__file__).resolve().parent / "fufi" / "fufi" / "fufi_menu.xml"
