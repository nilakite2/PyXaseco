...
Uses plugin_defaults.toml for local settings.
...
async def _fz_sync(aseco: 'Aseco', _data):
    global _fz
    try:
        section, path = get_plugin_section('plugin_freezone', getattr(aseco, '_base_dir', None))
        config: dict = {
            'ws_user': aseco.server.serverlogin,
            'ws_password': '',
            'testmode': True,
            'debugmode': False,
            'notify': 0,
            'notify_mute': '{1} has been forced to spectator (FreeZone).',
            'notify_unmute': '{1} is now allowed to play (FreeZone).',
        }
        if isinstance(section, dict):
            config.update({
                'testmode': as_bool(section.get('testmode'), config['testmode']),
                'debugmode': as_bool(section.get('debugmode'), config['debugmode']),
                'notify': as_int(section.get('notify'), config['notify']),
                'notify_mute': as_str(section.get('notify_mute'), config['notify_mute']),
                'notify_unmute': as_str(section.get('notify_unmute'), config['notify_unmute']),
            })
        _fz = FreezoneState(aseco, config)
        if path is not None:
            logger.info('[Freezone] Config loaded from %s', path)
