...
async def _on_local_record(aseco: 'Aseco', _rec):
    ...
    _state.player_local_digest.clear()
    _refresh_cp_targets_all(aseco)
    if _is_score_state(aseco):
        await _refresh_score_state_records(aseco, kind='local')
    else:
        await _draw_local_all(aseco)
...
async def _on_rpg_record(aseco: 'Aseco', _rec):
    ...
    _refresh_cp_targets_all(aseco)
    if _is_score_state(aseco):
        await _refresh_score_state_records(aseco, kind='dedi')
    else:
        await _draw_dedi_all(aseco)
...
def _is_score_state(aseco: 'Aseco') -> bool:
    mode = getattr(getattr(aseco, 'server', None), 'gameinfo', None)
    current_mode = getattr(mode, 'mode', -1)
    return _state.challenge_show_next or current_mode == getattr(Gameinfo, 'SCOR', 7)
...
async def _refresh_score_state_records(aseco: 'Aseco', kind: str = 'dedi') -> None:
    ...
    if kind == 'local':
        xml = build_local_records_for_score(aseco)
    else:
        xml = build_dedi_records_for_score(aseco)
