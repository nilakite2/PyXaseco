...
async def init_jfreu(aseco: 'Aseco', _param):
    ...
    # Load config
    _load_config(aseco)
    _read_lists_toml(aseco)
    await _read_guest_list(aseco)
    _read_bans_toml(aseco)
...
def _load_config(aseco: 'Aseco'):
    try:
        ...
        _state.servername        = str(g('servername', ''))
        _state.top               = str(g('servertop', ''))
        _state.autochangename    = b('autochangename')
        _state.infomessages      = int(g('infomessages', 1))
        _state.badwords          = b('badwords')
        _state.badwordsban       = b('badwordsban')
        _state.badwordsnum       = int(g('badwordsnum', 3))
        _state.badwordstime      = int(g('badwordstime', 10))
        _state.unspecvote        = b('unspecvote', True)
        _state.novote            = b('novote')

        _state.ranklimit         = b('ranklimit')
        _state.limit             = int(g('limit', 500000))
        _state.hardlimit         = int(g('hardlimit', 0))
        _state.autorank          = b('autorank')
        _state.offset            = int(g('offset', 0))
        _state.autolimit         = int(g('autolimit', _state.limit))
        _state.autorankminplayers = int(g('autorankminplayers', 10))
        _state.autorankvip       = b('autorankvip')
        _state.maxplayers        = int(g('maxplayers', 20))
        _state.kickhirank        = b('kickhirank')
        _state.pf                = int(g('pf', 0))
...
def _load_messages_from_config(data: dict):
    ...
    msg_block = data.get('jfreu_messages', {})
    if msg_block:
        def g(key, default=''):
            return msg_block.get(key.lower(), default)
        _state.player_join  = str(g('player_join', _state.player_join))
        _state.player_joins = str(g('player_joins', _state.player_joins))
        _state.player_left  = str(g('player_left', _state.player_left))
        _state.message_start = str(g('message_start', _state.message_start))
...
def _write_config_toml():
    try:
        ...
        data['jfreu_plugin'] = {
            ...
            'servertop': j.top,
            'autochangename': j.autochangename,
            'infomessages': j.infomessages,
            'badwords': j.badwords,
            'badwordsban': j.badwordsban,
            'badwordsnum': j.badwordsnum,
            'badwordstime': j.badwordstime,
            'unspecvote': j.unspecvote,
            'novote': j.novote,
            'ranklimit': j.ranklimit,
            'limit': j.limit,
            'hardlimit': j.hardlimit,
            'autorank': j.autorank,
            'offset': j.offset,
            'autolimit': j.autolimit,
            'autorankminplayers': j.autorankminplayers,
            'autorankvip': j.autorankvip,
            'maxplayers': j.maxplayers,
            'kickhirank': j.kickhirank,
            'pf': j.pf,
        }
        data['jfreu_messages'] = {
            'player_join': j.player_join,
            'player_joins': j.player_joins,
            'player_left': j.player_left,
            'message_start': j.message_start,
        }
...
async def _show_join_message(aseco: 'Aseco', player: 'Player', nation: str):
    ...
    try:
        from apps.players.rankings import _rasp_messages, feature_ranks
        rank_info = None
        if feature_ranks:
            pool = await localdb_get_pool(aseco)
            if pool:
                pid = await localdb_get_player_id(aseco, player.login)
...
def _normalize_join_nation(player: 'Player') -> str:
    ...
    if len(nation) > 14:
        return map_country(None, nation)
