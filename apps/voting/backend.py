...
VOTING_SETTINGS_SCHEMA = AppSettingsSchema(
    app_id='voting',
    description='Typed app defaults for voting and match-flow behaviour.',
    settings=(
...
async def _vote_panels_off(aseco: 'Aseco'):
    try:
        ...
        from apps.platform_ui.panels import allvotepanels_off
...
async def _vote_panels_on(aseco: 'Aseco', login: str):
    try:
        ...
        from apps.platform_ui.panels import allvotepanels_on
...
async def _broadcast_tmxadd(aseco: 'Aseco', msg: str):
    try:
        from apps.jukebox.jukebox import jukebox_in_window
    except Exception:
...
async def _init_votes(aseco: 'Aseco', _data):
    ...
    try:
        s = bind_app_settings(VOTING_SETTINGS_SCHEMA, aseco._base_dir)
...
async def _on_player_info_changed(aseco: 'Aseco', player) -> None:
    ...
    if is_spec:
        if not allow_spec_voting and not aseco.is_any_admin(player):
            try:
                from apps.platform_ui.panels import votepanel_off
        ...
        if (chatvote or tmxadd) and login not in plrvotes:
            try:
                from apps.platform_ui.panels import display_votepanel
