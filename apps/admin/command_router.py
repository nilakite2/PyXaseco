...
ADMIN_SETTINGS_SCHEMA = AppSettingsSchema(
    app_id="admin",
    section_name="admin",
    description="Admin command router settings",
    settings=(
        AppSetting("pm_buffer_len", 30, as_int, aliases=("chat_admin/pm_buffer_len",)),
        AppSetting("pm_line_len", 40, as_int, aliases=("chat_admin/pm_line_len",)),
        AppSetting("auto_scorepanel", True, as_bool, aliases=("chat_admin/auto_scorepanel",)),
        AppSetting("rounds_finishpanel", True, as_bool, aliases=("chat_admin/rounds_finishpanel",)),
    ),
)
...
def _load_admin_settings(aseco: 'Aseco') -> None:
    global PM_BUFFER_LEN, PM_LINE_LEN, AUTO_SCOREPANEL, ROUNDS_FINISHPANEL

    bound = bind_app_settings(ADMIN_SETTINGS_SCHEMA, getattr(aseco, "_base_dir", None))
    PM_BUFFER_LEN = int(bound.values["pm_buffer_len"] or PM_BUFFER_LEN)
    PM_LINE_LEN = int(bound.values["pm_line_len"] or PM_LINE_LEN)
    AUTO_SCOREPANEL = bool(bound.values["auto_scorepanel"])
    ROUNDS_FINISHPANEL = bool(bound.values["rounds_finishpanel"])

def register(aseco: 'Aseco'):
    _load_admin_settings(aseco)
    ...
    aseco.register_event('onChat_/', chat_admin)
...
def _visible_admin_commands_for_player(aseco: 'Aseco', player) -> list[tuple[str, str]]:
    ...
    for name, cc in visible_admin_commands_for_player(
        ...
        if display_name.strip().lower() == 'admin':
            continue
...
async def _deny_offline_target(aseco: 'Aseco', admin, target_login: str) -> None:
    await _reply(
        aseco,
        getattr(admin, 'login', ''),
...
def _get_bannedips_state(aseco: 'Aseco') -> list[str]:
    raw = getattr(getattr(aseco, 'settings', None), 'bannedips', []) or []
    seen: set[str] = set()
    result: list[str] = []
    for item in raw:
...
def _set_bannedips_state(aseco: 'Aseco', ips: list[str]) -> list[str]:
    normalized = []
    seen: set[str] = set()
    for item in ips:
        ip = str(item or '').strip()
    ...
    aseco.settings.bannedips = normalized
    return normalized
...
async def _write_bannedips_toml(aseco: 'Aseco') -> pathlib.Path | None:
    try:
        ...
        write_toml(path, {'banned_ips': ips})
        return path
    except Exception as e:
...
async def _read_bannedips_toml(aseco: 'Aseco') -> bool:
    try:
        ...
        if not isinstance(ips, list):
            return False
        old_ips = _get_bannedips_state(aseco)
        for ip in old_ips:
            try:
                await aseco.client.query_ignore_result('UnBanIP', ip)
            except Exception:
                pass
        normalized = _set_bannedips_state(aseco, ips)
...
async def _find_track_uid_by_filename(aseco: 'Aseco', fname: str) -> str:
    ...
    try:
        from apps.jukebox.jukebox import _parse_gbx_metadata
...
async def _remove_track_from_rotation(aseco: 'Aseco', fname: str, uid: str = ''):
    ...
    try:
        from apps.jukebox.jukebox import (
