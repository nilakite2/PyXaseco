...
STALKER_TOOLS_SETTINGS_SCHEMA = AppSettingsSchema(
    app_id='admin',
    section_name='admin',
    description="Optional STALKER's Tools settings",
    settings=(
        AppSetting('stalker_tools/enabled', False, as_bool, "Enable the optional STALKER's Tools extension.", 'stalker'),
    ),
)
...
feature_stalker_tools = False
_st_state: "_StalkerToolsState | None" = None


def _apply_app_defaults(aseco: "Aseco") -> None:
    global feature_stalker_tools

    bound = bind_app_settings(STALKER_TOOLS_SETTINGS_SCHEMA, getattr(aseco, '_base_dir', None))
    feature_stalker_tools = bool(bound.values['stalker_tools/enabled'])
    setattr(aseco, 'feature_stalker_tools', feature_stalker_tools)
    setattr(aseco.server, 'feature_stalker_tools', feature_stalker_tools)
...
def register(aseco: "Aseco"):
    ...
    aseco.add_chat_command(
        "st",
        "STALKER's Tools commands (see: /st help)",
        is_admin=True,
        owner='chat/admin',
        usage='/st <subcommand>',
        app='admin',
...
async def chat_chatall(aseco: "Aseco", command: dict):
    ...
    if not (
        ...
        await aseco.client.query_ignore_result("ChatSendServerMessageToLogin", "$f00You don't have the required admin rights to do that!", player.login)
        return
    ...
    if sub == "cc":
        if rest.strip() == "all":
            if not _is_masteradmin(aseco, player):
                ...
                return
            try:
                from apps.platform_core import chatlog as chatlog_mod
...
class StalkerToolsSurface(Component):
    def __init__(self):
        super().__init__(
            component_id='admin.stalker_tools',
            description="Optional STALKER's Tools admin extension.",
        )
...
SURFACE = StalkerToolsSurface()


def get_component() -> StalkerToolsSurface:
    return SURFACE
