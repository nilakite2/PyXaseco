...
def register(aseco: 'Aseco'):
    ...
    for order, (name, help_text, _flag) in enumerate(admin_cmds, start=100):
        aseco.register_command(
            f'admin/{name}',
            help_text,
            is_admin=True,
            owner='chat/admin',
            app='admin',
            category='admin-subcommand',
            parent='admin',
            usage=f'/admin {name}',
            display_name=name,
            public=name in {'help', 'helpall', 'listmasters', 'listadmins', 'listops'},
            permission=name,
            order=order,
        )
...
async def chat_admin(aseco: 'Aseco', command: dict):
    ...
    # ---- Player moderation ----
        ...
        if ip:
            try:
                await aseco.client.query_ignore_result('BanIP', ip)
                await _write_bannedips_toml(aseco)
    ...
    # ---- List commands ----
        ...
        try:
            path = await _write_bannedips_toml(aseco)
            if path:
                await _reply(aseco, login, '{#server}> Banned IPs list saved to {#highlite}bannedips.toml')
            else:
        ...
        try:
            ok = await _read_bannedips_toml(aseco)
            if ok:
                await _reply(aseco, login, '{#server}> Banned IPs list loaded from {#highlite}bannedips.toml')
            else:
    ...
    # ---- Admin / operator management ----
        ...
        path = _write_adminops_toml(aseco)
        if path:
            await _reply(
                aseco,
                login,
