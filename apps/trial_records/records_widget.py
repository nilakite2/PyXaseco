...
from apps.records_eyepiece.utils import _handle_special_chars, _safe_ml_text
...
async def _get_trial_plugin():
    mod = sys.modules.get('apps.trial_records.service')
    if mod is not None:
        return mod
    try:
        from apps.trial_records import service as mod
        return mod
    except Exception:
