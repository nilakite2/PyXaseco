...
from . import ranking_backend as _impl
...
def register_commands(aseco) -> None:
    aseco.add_chat_command('rank', 'Shows your current server rank', app='players', category='chat-players')
    aseco.add_chat_command('top10', 'Displays top 10 best ranked players', app='players', category='chat-players')
    aseco.add_chat_command('top100', 'Displays top 100 best ranked players', app='players', category='chat-players')
    aseco.add_chat_command('topwins', 'Displays top 100 victorious players', app='players', category='chat-players')
    aseco.add_chat_command('active', 'Displays top 100 most active players', app='players', category='chat-players')
...
class RaspRankingCommandSurface(Component):
    def __init__(self):
        super().__init__(
            component_id='players.rankings.commands',
...
class RaspRankingCallbackSurface(Component):
    def __init__(self):
        super().__init__(
            component_id='players.rankings.callbacks',
            description='Player ranking lifecycle, finish, and leaderboard callback surface.',
            callbacks=(
