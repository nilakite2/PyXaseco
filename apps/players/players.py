...
def _map_country(aseco: 'Aseco', nation: str) -> str:
    service = aseco.get_service('localdb') or aseco.get_service('service/localdb')
    mapper = getattr(service, 'map_country', None) if service is not None else None
    if callable(mapper):
        return mapper(nation)
    return nation[:3].upper() if nation else ''


def _display_country(aseco: 'Aseco', pl) -> str:
    ...
    if len(country) > 14:
        mapped = _map_country(aseco, country)
        return mapped

    return country


def _display_country_values(aseco: 'Aseco', nation: str, zone: str = '') -> str:
    class _CountryView:
        pass
    ...
    view.nation = nation
    view.zone = zone
    return _display_country(aseco, view)
...
async def chat_players(aseco: 'Aseco', command: dict):
    ...
    if db_rows:
        for row in db_rows:
            ...
            nat = _display_country_values(
                aseco,
                getattr(online, 'nation', '') or nation_db,
                getattr(online, 'zone', '') or '',
            )
...
async def event_players(aseco: 'Aseco', answer: list):
    ...
    if 2001 <= action <= 2200:
        ...
        if idx < len(player.playerlist):
            ...
            from apps.players.stats import chat_stats
