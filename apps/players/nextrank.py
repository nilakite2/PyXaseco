...
async def chat_nextrank(aseco: 'Aseco', command: dict):
    ...
    try:
        from apps.players.rankings import (
    ...
    player_id = getattr(player, 'id', 0) or await localdb_get_player_id(aseco, login)
    if not player_id:
        msg = format_text(
...
def register_commands(aseco: 'Aseco') -> None:
    aseco.add_chat_command('nextrank', 'Shows the next better ranked player', app='players', category='chat-players')
    aseco.register_event('onChat_nextrank', chat_nextrank)


class NextRankCommandSurface(Component):
    def __init__(self):
        super().__init__(
            component_id='players.rankings.nextrank.commands',
            description='Next-rank command surface owned by the players app.',
            commands=(
                Command('nextrank', 'Shows the next better ranked player', chat_nextrank),
            ),
        )
...
NEXTRANK_COMMAND_SURFACE = NextRankCommandSurface()


def get_command_component() -> NextRankCommandSurface:
    return NEXTRANK_COMMAND_SURFACE
