...
from pyxaseco.app_services import get_dedi_db, localdb_get_pool
...
def register(aseco: 'Aseco'):
    for order, (name, help_text) in enumerate([
        ...
        aseco.add_chat_command(
            name,
            help_text,
            owner='chat/dedimania',
            app='records_dedimania',
            category='chat-dedimania',
            usage=f'/{name}',
            display_name=name,
            order=order,
        )
...
def _get_dedi_db(aseco: 'Aseco') -> dict:
    return get_dedi_db(aseco)


def _records(aseco: 'Aseco') -> list[dict]:
    db   = _get_dedi_db(aseco)
    recs = db.get('Challenge', {}).get('Records', [])
    return recs if isinstance(recs, list) else []


def _server_maxrank(aseco: 'Aseco') -> int:
    db = _get_dedi_db(aseco)
    return int(db.get('MaxRank', db.get('ServerMaxRank', 30)) or 30)


def _dedi_messages(aseco: 'Aseco') -> dict:
    return _get_dedi_db(aseco).get('Messages', {})


def _msg(aseco: 'Aseco', key: str, default: str) -> str:
    """Get message template from dedi_db['Messages'] with fallback."""
    val = _dedi_messages(aseco).get(key, [default])
    return val[0] if isinstance(val, list) and val else default


def _show_rec_logins(aseco: 'Aseco') -> bool:
    return bool(_get_dedi_db(aseco).get('ShowRecLogins', False))


def _show_recs_range(aseco: 'Aseco') -> bool:
    return bool(_get_dedi_db(aseco).get('ShowRecsRange', False))


def _show_min_recs(aseco: 'Aseco') -> int:
    return int(_get_dedi_db(aseco).get('ShowMinRecs', 8) or 8)
...
def _find_player_record(aseco: 'Aseco', login: str) -> tuple[int, dict] | tuple[None, None]:
    ...
    for idx, rec in enumerate(_records(aseco)):
...
async def show_dedirecs(aseco: 'Aseco', name: str, uid: str,
    ...
    # Compute range diff
    range_diff = None
    if _show_recs_range(aseco) and total > 0:
        first = dedi_recs[0]
    ...
    if totalnew > 0:
        message = format_text(_msg(aseco, 'RANKING_NEW', aseco.get_chat_message('RANKING_NEW')),
                              track_name, timing, totalnew)
    elif totalnew == 0 and has_recs:
        if _show_recs_range(aseco) and range_diff is not None:
            message = format_text(
                _msg(aseco, 'RANKING_RANGE', aseco.get_chat_message('RANKING_RANGE')),
                track_name, timing, range_diff)
        else:
            message = format_text(
                _msg(aseco, 'RANKING', aseco.get_chat_message('RANKING')),
                track_name, timing)
    elif totalnew == 0 and not has_recs:
        message = format_text(
            _msg(aseco, 'RANKING_NONEW', aseco.get_chat_message('RANKING_NONEW')),
            track_name, timing)
    else:
        message = format_text(
            _msg(aseco, 'RANKING_NONE', aseco.get_chat_message('RANKING_NONE')),
            track_name, timing)
...
async def chat_dedirecs(aseco: 'Aseco', command: dict):
    ...
    maxrank     = max(_server_maxrank(aseco), int(getattr(player, 'dedirank', 0) or 0))
    show_logins = _show_rec_logins(aseco)
...
async def chat_dedinew(aseco: 'Aseco', command: dict):
    ...
    await show_dedirecs(aseco, ch.name, ch.uid, _records(aseco),
...
async def chat_dedipb(aseco: 'Aseco', command: dict):
    ...
    msg   = format_text(_msg(aseco, 'PB', '{#server}> {#dedirec}Dedimania Personal Best: '
...
async def chat_dedifirst(aseco: 'Aseco', command: dict):
    ...
    if not recs:
        await _send(aseco, login, '{#server}> {#error}No Dedimania records found!')
        return
    prefix = format_text(_msg(aseco, 'FIRST_RECORD', '{#server}> {#dedirec}The first Dedimania record is: '))
...
async def chat_dedilast(aseco: 'Aseco', command: dict):
    ...
    if not recs:
        ...
        return
    pos    = len(recs)
    prefix = format_text(_msg(aseco, 'LAST_RECORD', '{#server}> {#dedirec}The last Dedimania record is: '))
...
async def chat_dedinext(aseco: 'Aseco', command: dict):
    ...
    if rec is not None:
        ...
        msg  = format_text(_msg(aseco, 'DIFF_RECORD', aseco.get_chat_message('DIFF_RECORD')),
                           msg1, msg2, diff_str)
        await _send(aseco, login, msg)
    else:
...
async def chat_dedistats(aseco: 'Aseco', command: dict):
    ...
    stats  = [
        ['Server MaxRank', f'{{#black}}{_server_maxrank(aseco)}'],
        ['Your MaxRank',   f'{{#black}}{int(getattr(player, "dedirank", 0) or _server_maxrank(aseco))}'],
        [],
        ['UID',            f'{{#black}}{uid}'],
