...
async def chat_afk(aseco: 'Aseco', command: dict):
    ...
    if aseco.settings.afk_force_spec and not _p_is_spec:
        try:
            ...
            if max_spec == 0:
                logger.warning('[social_chat] /afk: server MaxSpectators=0, cannot force spec')
                return
            ...
            logger.warning('[social_chat] /afk ForceSpectator failed for %s: %s',
                           p.login, _afk_err)
...
async def chat_official(aseco: 'Aseco', command: dict):
    try:
        from apps.players.rankings import _rasp_messages
