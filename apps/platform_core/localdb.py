...
PLUGIN_METADATA = {
    ...
    'description': 'Provides local database access, player/challenge persistence, and local record state.',
    'provides': ['service/localdb', 'localdb'],
}
...
def register(aseco: 'Aseco'):
    aseco.register_service(
        'service/localdb',
        sys.modules[__name__],
        owner='core/localdb',
        aliases=['localdb', 'core/localdb'],
    )
...
async def ldb_load_settings(aseco: 'Aseco', _param):
    ...
    config_path = aseco._base_dir / 'settings.toml'
    aseco.console('[LocalDB] Load config file [{1}]', str(config_path))

    _load_dotenv(aseco._base_dir / '.env')
    _load_dotenv('.env')
    ...
    try:
        ...
        overlay_section(_messages, 'local_messages', strip_prefix='local_', base_dir=aseco._base_dir)
    except Exception as exc:
        logger.warning('[LocalDB] messages config overlay failed: %s', exc)

    try:
        from pyxaseco.settings_loader import overlay_localdatabase
        display_box = [_display]
        limit_box = [_limit]
        overlay_localdatabase(display_box, limit_box, base_dir=aseco._base_dir)
        _display, _limit = display_box[0], limit_box[0]
    except Exception as exc:
        logger.warning('[LocalDB] settings config overlay failed: %s', exc)
...
def _get_msg(key: str, *args) -> str:
    """Look up a message from the LocalDB message config and format it."""
