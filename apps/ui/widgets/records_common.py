...
def _build_record_widget(
    *,
    ...
    line_h = max(1.0, getattr(cfg, 'row_height', _state.line_height))
    widget_h = getattr(cfg, 'height', 0.0) or (line_h * cfg.entries + 3.3)
    col_h = widget_h - 3.1
    col_name_w = w - 6.45
    ...
    bg_attrs = (
        f'bgcolor="{_safe_ml_text(cfg.bg_color)}"'
        if getattr(cfg, 'bg_color', '')
