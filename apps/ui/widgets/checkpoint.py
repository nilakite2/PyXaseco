...
async def _draw_cp_player(aseco: 'Aseco', login: str):
    ...
    st = _state.style
    cp_cfg = _state.cp
    width = max(1.0, float(getattr(cp_cfg, 'width', 16.0) or 16.0))
    height = max(1.0, float(getattr(cp_cfg, 'height', 4.0) or 4.0))
    center_x = width / 2.0
    ch = aseco.server.challenge
    mode = _effective_mode(aseco)
    ...
    bg_attrs = (
        f'bgcolor="{cp_cfg.bg_color}"'
        if getattr(cp_cfg, 'bg_color', '')
        else f'style="{st.bg_style}" substyle="{st.bg_substyle}"'
    )
    ...
    xml = (
        ...
        f'<quad posn="0 0 0.001" sizen="{width:.4f} {height:.4f}"'
        f' {bg_attrs}/>'
        f'<label posn="{center_x:.4f} -0.65 0.01" halign="center"'
        f' textsize="1" scale="0.6" textcolor="{top_color}"'
        f' text="{top_text}"/>'
    )
    if bot_text:
        xml += (
            f'<label posn="{center_x:.4f} -1.8 0.01" halign="center"'
