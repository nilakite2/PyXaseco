...
def _scoretable_header(ml_id: int, cfg: dict, widget_height: float) -> str:
    ...
    bg_color    = getattr(st, 'score_bg_color',    '')
    ...
    width = max(15.5, float(cfg.get('width', 15.5) or 15.5))
...
def _scoretable_row(line: int, rank: int, score_text: str, nick: str,
                    score_color: str, fmt: str, lh: float,
                    name_width: float = 10.2) -> str:
    """Single entry row"""
    offset = 3.0
    y = lh * line + offset
    ...
    p.append(f'<label posn="5.9 -{y:.2f} 0.002" sizen="{name_width:.2f} 1.7"'
             f' scale="0.9"'
...
def build_local_records_for_score(aseco: 'Aseco') -> str:
    ...
    lh     = float(cfg.get('row_height', _state.line_height) or _state.line_height)
    ...
    widget_height = float(cfg.get('height', 0.0) or (lh * min(len(records), entries) + 3.3))
    p = [_scoretable_header(ML_LOCAL_SCORE, cfg, widget_height)]
    name_width = max(float(cfg.get('width', 15.5) or 15.5) - 5.3, 2.0)
    for idx, (rank, score_text, nick) in enumerate(records[:entries]):
        p.append(_scoretable_row(idx, rank, score_text, nick, sc, fmt, lh, name_width))
...
def build_dedi_records_for_score(aseco: 'Aseco') -> str:
    ...
    score_cfg = dict(cfg)
    lh     = float(score_cfg.get('row_height', _state.line_height) or _state.line_height)
    entries= int(cfg.get('entries', 8))
    fmt    = getattr(_state.style, 'score_fmt_codes', '')
    sc     = getattr(_state.style, 'score_col_scores', 'DDDF')
    ...
    try:
        ...
        try:
            import sys as _sys
            mod = (
                _sys.modules.get('apps.records_rpg.service')
                or _sys.modules.get('apps.records_rpg.service')
            )
        ...
        if isinstance(rpg_track, dict) and rpg_rows:
            ...
            source_rows = _get_dedi_records(aseco) or []
    ...
    widget_height = float(score_cfg.get('height', cfg.get('height', 0.0)) or (lh * len(records) + 3.3))
    p = [_scoretable_header(ML_DEDI_SCORE, score_cfg, widget_height)]
    name_width = max(float(score_cfg.get('width', 15.5) or 15.5) - 5.3, 2.0)
...
def build_avg_times_for_score(aseco: 'Aseco') -> str:
    ...
    widget_height = float(cfg.get('height', 0.0) or (lh * min(len(data), entries) + 3.3))
    ...
    for idx, item in enumerate(data[:entries]):
        ...
        p.append(_scoretable_row(idx, idx + 1, score_text,
                                 item['nickname'], sc, fmt, lh, name_width))
    p.append(_scoretable_footer())
    return ''.join(p)
...
def build_donation_widget(aseco: 'Aseco') -> str:
    ...
    width    = max(4.6, float(cfg.get('width', 4.6) or 4.6))
    row_h    = max(1.0, float(cfg.get('row_height', 1.85) or 1.85))
    widget_height = float(cfg.get('height', 0.0) or (6.55 + len(amounts) * row_h))
    title_center = width / 2.0
    icon_x = max(0.2, (width - 3.2) / 2.0)
    button_x = 0.2
    button_w = max(1.0, width - 0.4)
    ...
    p.append(f'<quad posn="{icon_x:.2f} -0.3 0.002" sizen="3.2 2.7"'
             f' style="{escape(icon_sty)}" substyle="{escape(icon_sub)}"/>')
    p.append(f'<label posn="{title_center:.2f} -3.4 0.1" sizen="{max(width - 0.95, 1.0):.2f} 2"'
             f' halign="center" scale="0.9" text="PLEASE"/>')
    p.append(f'<label posn="{title_center:.2f} -4.9 0.1" sizen="{max(width + 1.75, 1.0):.2f} 2"'
    ...
    for i, amount in enumerate(amounts):
        row = i * row_h
        action = 918165 + i
        p.append(f'<quad posn="{button_x:.2f} -{offset+row:.2f} 0.2" sizen="{button_w:.2f} 1.7"'
                 f' action="{action}"'
                 f' style="{escape(btn_style)}" substyle="{escape(btn_sub)}"/>')
        p.append(f'<label posn="{title_center:.2f} -{offset+row+0.35:.2f} 0.3"'
                 f' sizen="{max(width - 0.6, 1.0):.2f} 2.5" halign="center" scale="0.8"'
                 f' text="{amount}$n $mC"/>')
...
def build_winning_payout_widget(aseco: 'Aseco') -> str:
    ...
    lh       = max(1.0, float(cfg.get('row_height', _state.line_height) or _state.line_height))
    st       = _state.style
    ...
    width     = max(25.5, float(cfg.get('width', 25.5) or 25.5))
    bg_style  = cfg.get('bg_style', 'BgsPlayerCard')
    bg_sub    = cfg.get('bg_substyle', 'BgRacePlayerName')
    bg_color  = cfg.get('bg_color', '')
    ...
    widget_height = float(cfg.get('height', 0.0) or (lh * 3 + 3.4))
    finishers = []
    try:
    ...
    if bg_color:
        ...
        p.append(f'<quad posn="0 0 0.001" sizen="{width:.2f} {widget_height:.2f}"'
                 f' style="{escape(bg_style)}" substyle="{escape(bg_sub)}"/>')
    p.append(f'<quad posn="0.4 -0.36 0.002" sizen="{max(width - 0.8, 1.0):.2f} 2"'
             f' style="{escape(ttl_style)}" substyle="{escape(ttl_sub)}"/>')
    p.append(f'<quad posn="0.6 0 0.004" sizen="2.5 2.5"'
             f' style="{escape(icon_sty)}" substyle="{escape(icon_sub)}"/>')
    p.append(f'<label posn="3.2 -0.55 0.004" sizen="{max(width - 5.3, 1.0):.2f} 0"'
    ...
    for i, item in enumerate(finishers):
        ...
        p.append(f'<label posn="6.2 -{y:.2f} 0.002" sizen="3.95 1.7"'
                 ...
                 f' text="+{won} C"/>')
        p.append(f'<label posn="6.5 -{y:.2f} 0.002" sizen="{max(width - 14.6, 1.0):.2f} 1.7"'
                 f' scale="0.9" text="{nick}"/>')
        if won > 0:
            p.append(f'<label posn="{max(width - 1.0, 8.0):.2f} -{y:.2f} 0.002" sizen="8 1.7"'
...
def build_round_score_widget(aseco: 'Aseco') -> str:
    ...
    lh      = max(1.0, float(phase_cfg.get('row_height', _state.line_height) or _state.line_height))
    ...
    widget_height = float(rs_cfg.get('height', 0.0) or (lh * entries + 3.3))
