...
def _parse_gm_widgets(root, key: str, default_title: str, *, live: bool = False) -> dict:
    ...
    for name, mode in mapping.items():
        ...
        height = max(0.0, _get_float(block, 'HEIGHT', 0.0))
        entries = max(1, _get_int(sub, 'ENTRIES', 8))
        topcount = max(1, _get_int(sub, 'TOPCOUNT', 3))
        row_height = max(1.0, _get_float(sub, 'ROW_HEIGHT', PLUGIN_LINE_HEIGHT))
        ...
        cfg = WidgetCfg(
            ...
            bg_color=str(_get_node_value(sub, 'BACKGROUND_COLOR', '')),
            entries=entries,
            topcount=topcount,
...
def _load_config(aseco: 'Aseco') -> None:
    ...
    raw = {}
    try:
        raw, path = find_and_load(base_dir)
    ...
    links_block = _get_node_value(root, 'LINKS', {})
    _state.links = {
        'website': str(_get_node_value(links_block, 'WEBSITE', '')),
        'discord': str(_get_node_value(links_block, 'DISCORD', '')),
        'welcome': str(_get_node_value(links_block, 'WELCOME', '')),
    ...
    _state.style = StyleCfg(
        ...
        bg_color=str(_get_node_value(widget_race, 'BACKGROUND_COLOR', '')),
    ...
    _state.style.score_bg_color    = str(_get_node_value(widget_score, 'BACKGROUND_COLOR', ''))
    ...
    _state.cp = WidgetCfg(
        ...
        width=_get_float(cp, 'WIDTH', 16.0),
        height=_get_float(cp, 'HEIGHT', 4.0),
    ...
    def _bar(xml_key, defaults):
        ...
        return BarWidgetCfg(
            ...
            width=max(1.0, _get_float(blk, 'WIDTH', defaults.get('width', 4.6))),
            height=max(1.0, _get_float(blk, 'HEIGHT', defaults.get('height', 6.5))),
            ...
            bg_color=str(_get_node_value(blk, 'BACKGROUND_COLOR', defaults.get('bg_color', ''))),
        )
    ...
    _state.favorite = FavoriteWidgetCfg(
        ...
        width=max(1.0, _get_float(fav_blk, 'WIDTH', 4.6)),
        height=max(1.0, _get_float(fav_blk, 'HEIGHT', 6.5)),
        ...
        race_bg_color=str(_get_node_value(fav_race, 'BACKGROUND_COLOR', '')),
        ...
        score_bg_color=str(_get_node_value(fav_score, 'BACKGROUND_COLOR', '')),
    )
    ...
    _state.clock = ClockWidgetCfg(
        ...
        width=max(1.0, _get_float(clk_blk, 'WIDTH', 4.6)),
        height=max(1.0, _get_float(clk_blk, 'HEIGHT', 6.5)),
        ...
        race_bg_color=str(_get_node_value(clk_race, 'BACKGROUND_COLOR', '')),
        ...
        score_bg_color=str(_get_node_value(clk_score, 'BACKGROUND_COLOR', '')),
    )
    ...
    _state.next_env = NextEnvWidgetCfg(
        ...
        width=max(1.0, _get_float(nev_blk, 'WIDTH', 4.6)),
        height=max(1.0, _get_float(nev_blk, 'HEIGHT', 6.5)),
        ...
        bg_color=str(_get_node_value(nev_blk, 'BACKGROUND_COLOR', '')),
    )
    ...
    _state.next_gamemode_widget = NextGamemodeWidgetCfg(
        ...
        width=max(1.0, _get_float(ngm_blk, 'WIDTH', 4.6)),
        height=max(1.0, _get_float(ngm_blk, 'HEIGHT', 6.5)),
        ...
        bg_color=str(_get_node_value(ngm_blk, 'BACKGROUND_COLOR', '')),
    )
    ...
    _state.eyepiece_widget = EyepieceWidgetCfg(
        width=max(1.0, _get_float(ep_blk, 'WIDTH', 4.6)),
        height=max(1.0, _get_float(ep_blk, 'HEIGHT', 6.5)),
        ...
        score_bg_color=str(_get_node_value(ep_score, 'BACKGROUND_COLOR', '')),
    )
    ...
    def _rs_gm(tag: str) -> dict:
        ...
        return {
            'enabled': enabled,
            'race': {
                'pos_x': _get_float(race_n, 'POS_X', 49.2),
                'pos_y': _get_float(race_n, 'POS_Y', 17.8),
                'entries': _get_int(race_n, 'ENTRIES', 14),
                'topcount': _get_int(race_n, 'TOPCOUNT', 3),
                'row_height': max(1.0, _get_float(race_n, 'ROW_HEIGHT', PLUGIN_LINE_HEIGHT)),
            },
            'warmup': {
                'pos_x': _get_float(warm_n, 'POS_X', 49.2),
                'pos_y': _get_float(warm_n, 'POS_Y', 10.7),
                'entries': _get_int(warm_n, 'ENTRIES', 10),
                'topcount': _get_int(warm_n, 'TOPCOUNT', 3),
                'row_height': max(1.0, _get_float(warm_n, 'ROW_HEIGHT', PLUGIN_LINE_HEIGHT)),
            },
        }
    ...
    _state.round_score_cfg = {
        ...
        'height': max(0.0, _get_float(rs_node, 'HEIGHT', 0.0)),
    ...
    def _stl(key: str, d: dict) -> dict:
        ...
        return {
            'enabled': _get_bool(node, 'ENABLED', d.get('enabled', True)),
            'title': str(_get_node_value(node, 'TITLE', d.get('title', key))),
            'pos_x': _get_float(node, 'POS_X', d.get('pos_x', 0.0)),
            'pos_y': _get_float(node, 'POS_Y', d.get('pos_y', 0.0)),
            'width': max(15.5, _get_float(node, 'WIDTH', d.get('width', 15.5))),
            'height': max(0.0, _get_float(node, 'HEIGHT', d.get('height', 0.0))),
            'entries': _get_int(node, 'ENTRIES', d.get('entries', 6)),
            'row_height': max(1.0, _get_float(node, 'ROW_HEIGHT', d.get('row_height', PLUGIN_LINE_HEIGHT))),
            'icon_style': str(_get_node_value(node, 'ICON_STYLE', d.get('icon_style', 'Icons128x128_1'))),
            'icon_substyle': str(_get_node_value(node, 'ICON_SUBSTYLE', d.get('icon_substyle', 'Rankings'))),
        }
    ...
    _state.donation_cfg = {
        ...
        'width': max(4.6, _get_float(don_wid, 'WIDTH', 4.6)),
        'height': max(0.0, _get_float(don_wid, 'HEIGHT', 0.0)),
        'row_height': max(1.0, _get_float(don_wid, 'ROW_HEIGHT', 1.85)),
        ...
        'bg_color': str(_get_node_value(don_wid,'BACKGROUND_COLOR','')),
    ...
    _state.winning_payout_cfg = {
        ...
        'width': max(25.5, _get_float(wp_wid, 'WIDTH', 25.5)),
        'height': max(0.0, _get_float(wp_wid, 'HEIGHT', 0.0)),
        'row_height': max(1.0, _get_float(wp_wid, 'ROW_HEIGHT', 1.8)),
        ...
        'bg_color': str(_get_node_value(wp_wid,'BACKGROUND_COLOR','')),
...
def _loaded_plugins(aseco: 'Aseco') -> list[str]:
    if getattr(aseco, '_app_manager', None):
        fulfilled = getattr(aseco._app_manager, 'fulfilled_entries', None)
        if fulfilled is not None:
            return list(fulfilled() if callable(fulfilled) else fulfilled or [])
        return list(getattr(aseco._app_manager, 'loaded_entries', []) or [])
    return list(getattr(aseco, '_apps', []) or [])
...
def validate_phase1_dependencies(aseco: 'Aseco') -> None:
    ...
    required_groups = [
        ('core/localdb',),
        ('service/tmx', 'app/tmx'),
        ('feature/rasp_jukebox', 'app/jukebox'),
    ]
    if any(cfg.enabled for cfg in _state.dedi.values()):
        required_groups.append(('service/records_dedimania', 'app/records_dedimania'))

    missing = sorted(group[0] for group in required_groups if not any(name in loaded for name in group))
    if missing:
        raise RuntimeError(
            f'[{PLUGIN_NAME}] Unmet requirements! Missing app/loadout requirement(s): ' + ', '.join(missing)
        )
