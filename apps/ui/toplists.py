...
def _load_nations_toml(aseco: 'Aseco') -> dict[str, str]:
    global _COUNTRY_CACHE
    if _COUNTRY_CACHE is not None:
        return _COUNTRY_CACHE
    candidates = [
        Path(getattr(aseco, '_base_dir', '.')).resolve() / 'nations.toml',
        Path('.').resolve() / 'nations.toml',
    ]
    result: dict[str, str] = {}
...
def _country_name(aseco: 'Aseco', code: str) -> str:
    ...
    return _load_nations_toml(aseco).get(code, code)
...
async def _db_query(sql: str, params: tuple = ()) -> list[tuple]:
    ...
    try:
        pool = await localdb_get_pool(getattr(_state, 'aseco', None))
        if not pool:
            return []
...
async def _query_ranked_record_counts(aseco: 'Aseco', limit: int) -> list[tuple]:
    ...
    """
    try:
        from apps.records_local.chat_records2 import _get_challenge_list
    except Exception as exc:
