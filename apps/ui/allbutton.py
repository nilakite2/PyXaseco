...
from pyxaseco.app_config import AppSetting, AppSettingsSchema, as_bool, as_float, bind_app_settings
...
ALLBUTTON_SETTINGS_SCHEMA = AppSettingsSchema(
    app_id='ui',
    section_name='ui',
    description='Optional TGJ allbutton HUD settings',
    settings=(
        AppSetting('allbutton/enabled', False, as_bool, 'Enable the optional TGJ quick-chat HUD.', 'allbutton'),
        AppSetting('allbutton/main_pos_x', -61.0, as_float, 'Main button row X position.', 'allbutton'),
        AppSetting('allbutton/main_pos_y', -45.1, as_float, 'Main button row Y position.', 'allbutton'),
        AppSetting('allbutton/submenu_pos_x', -39.0, as_float, 'Secondary submenu X position.', 'allbutton'),
        AppSetting('allbutton/submenu_pos_y', -28.2, as_float, 'Secondary submenu Y position.', 'allbutton'),
    ),
)

feature_allbutton = False
_main_pos_x = -61.0
_main_pos_y = -45.1
_submenu_pos_x = -39.0
_submenu_pos_y = -28.2
...
def _apply_app_defaults(aseco: 'Aseco') -> None:
    global feature_allbutton, _main_pos_x, _main_pos_y, _submenu_pos_x, _submenu_pos_y

    bound = bind_app_settings(ALLBUTTON_SETTINGS_SCHEMA, getattr(aseco, '_base_dir', None))
    feature_allbutton = bool(bound.values['allbutton/enabled'])
    _main_pos_x = float(bound.values['allbutton/main_pos_x'])
    _main_pos_y = float(bound.values['allbutton/main_pos_y'])
    _submenu_pos_x = float(bound.values['allbutton/submenu_pos_x'])
    _submenu_pos_y = float(bound.values['allbutton/submenu_pos_y'])
    setattr(aseco, 'feature_allbutton', feature_allbutton)
    setattr(aseco.server, 'feature_allbutton', feature_allbutton)
...
async def _send_colored_chat(aseco: 'Aseco', player: 'Player', message: str):
    ...
    await aseco.client.query_ignore_result('ChatSendServerMessage', aseco.format_colors(chat))


def _main_xml() -> str:
    return (
        f"<manialink id='{MAIN_ML_ID}'>"
        f"<frame posn='{_main_pos_x} {_main_pos_y} 0'>"
...
def _submenu_xml() -> str:
    ...
    for idx, action in enumerate(sorted(SUB_ACTIONS)):
        color = '$08F' if idx % 2 == 0 else '$999'
        labels.append(
            f"<label posn='{idx * 2} -0.8 0' action='{action}' sizen='2 1' "
            f"style='TextCardSmallScores2Rank' text='$s{color}{idx}X'/>"
        )
    return (
        f"<manialink id='{SUB_ML_ID}'>"
        f"<frame posn='{_submenu_pos_x} {_submenu_pos_y} 0'>"
...
async def _allbutton_click(aseco: 'Aseco', answer: list):
    ...
    if action == ACTION_HIN:
        ...
        msg = f'$z$s[{player.nickname}$z] {{#interact}}$s$i$w$f00D$f90a$ff0y$0f0u$0cfu$00cu$63fm $00ci$0cft$0f0s $ff0d$f90a$f00t $z$s{target}'
