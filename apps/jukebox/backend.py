...
from pyxaseco.app_services import get_tmx_service, localdb_get_pool
...
def _tmx_service(aseco=None):
    return get_tmx_service(aseco or _runtime_aseco)


def _rasp_module():
    from apps.players import rankings as rankings_app
    return rankings_app


def _rasp_votes_module():
    from apps.voting import voting as voting_app
    return voting_app
...
def _apply_app_defaults(aseco: 'Aseco'):
    try:
        ...
        overlay_app_defaults(
            ...
            base_dir=getattr(aseco, '_base_dir', None),
        )
    except Exception:
        pass
...
def _get_rasp_msg(aseco: 'Aseco', key: str) -> str:
    try:
        _rasp = getattr(_rasp_module(), '_rasp')
        msgs = _rasp.get('messages', {})
        value = msgs.get(key.upper(), '{#server}> {#error}' + key)
        if isinstance(value, list):
            return value[0] if value else '{#server}> {#error}' + key
        if value is None:
...
async def _jb_broadcast(aseco: 'Aseco', msg: str, at_score: bool = False):
    if jukebox_in_window:
        try:
            from pyxaseco.helpers import send_window_message
...
async def _default_tmx_section(aseco: 'Aseco') -> str:
    ...
    impl = getattr(service, 'get_tmx_section', None) if service is not None else None
    return await impl(aseco) if callable(impl) else 'TMNF'


def _tmx_api_url(section: str, endpoint: str, **params) -> str:
    service = _tmx_service()
    impl = getattr(service, 'tmx_prefix_for_section', None) if service is not None else None
    prefix = impl(section) if callable(impl) else ''
    if not prefix:
...
def parse_tmx_reference(ref: str, source: str = '') -> tuple[str, int]:
    ...
    if ref.isdigit():
        service = _tmx_service()
        host_for_section = getattr(service, 'tmx_public_host_for_section', None) if service is not None else None
        if source:
            source = source.upper()
            if source not in TMX_SECTION_CHOICES:
                raise ValueError(f'Unsupported TMX source: {source}')
            return (host_for_section(source) if callable(host_for_section) else 'tmnf.exchange'), int(ref)
        return (host_for_section('TMNF') if callable(host_for_section) else 'tmnf.exchange'), int(ref)

    parsed = urllib.parse.urlparse(ref)
    host = parsed.netloc.lower()
    service = _tmx_service()
    host_for_section = getattr(service, 'tmx_public_host_for_section', None) if service is not None else None
    known_hosts = {host_for_section(section) for section in TMX_SECTION_CHOICES} if callable(host_for_section) else {'tmnf.exchange'}
...
async def admin_add_tmx_track(
    ...
    try:
        source_hint = (source or '').strip()
        if not source_hint:
            try:
                source_hint = await _default_tmx_section(aseco)
            except Exception:
                source_hint = ''

        abs_path, rel_insert, host, track_id, metadata = await download_tmx_track(ref, tracks_root, source_hint)
        ...
        if use_add_challenge:
            try:
                ...
                requester_nick = await _resolve_requester_nick(aseco, login)
                jukebox[uid] = {
...
async def _rasp_endrace(aseco: 'Aseco', _data):
    ...
    if jukebox:
        ...
        has_requester = bool(stripped_nick or str(next_track.get('Login', '')).strip())

        if is_tmu_env:
            if next_track.get('tmx') and not has_requester:
                logmsg = (
...
async def chat_add(aseco: 'Aseco', command: dict):
    ...
    try:
        from apps.platform_core.track import time_playing
...
async def _event_jukebox(aseco: 'Aseco', answer: list):
    ...
    if 101 <= action <= 2000:
        ...
        if track_idx < len(tracklist):
            try:
                from apps.ui.karma import chat_karma
        ...
        if aseco.allow_ability(player, 'dropjukebox'):
            ...
            try:
                from apps.admin.command_router import chat_admin
        ...
        if idx < len(tracklist):
            ...
            try:
                ...
                logger.debug('[XList] TMX info click failed for %s: %s', login, e)
        ...
        if idx < len(tracklist):
            ...
            if aseco.allow_ability(player, 'add'):
                try:
                    ...
                    logger.debug('[XList] Admin add click failed for %s: %s', login, e)
            else:
        ...
        if idx < len(tracklist):
            ...
            try:
                ...
                logger.debug('[XList] Add button click failed for %s: %s', login, e)
